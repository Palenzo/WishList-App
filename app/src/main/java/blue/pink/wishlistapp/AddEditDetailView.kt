...
@Suppress("EmptyIfStatement","EmptyElseBlock")
...
fun AddEditDetailView(
    ...
    if (
        ...
        viewModel.wishDesState = wish.value.description
        viewModel.wishTitleState = wish.value.title
    }else{
        viewModel.wishTitleState = ""
        viewModel.wishDesState = ""
    ...
    Scaffold(
        ...
        Column(
            ...
            WishTextField(
                label = "Title",
                value = viewModel.wishDesState,
                onValueChanged = {
                    viewModel.onWishDesChange(it)
                })
            Spacer(modifier = Modifier.height(10.dp))
            Button(
                onClick = {
                    if (viewModel.wishTitleState.isNotEmpty()&&viewModel.wishDesState.isNotEmpty()) {
                        if (id!=0L){
                            viewModel.updateWish(
                                Wish(
                                    id = id,
                                    title = viewModel.wishTitleState.trim(),
                                    description = viewModel.wishDesState.trim()
                                )
                            )
                        }else{
                            viewModel.addWish(
